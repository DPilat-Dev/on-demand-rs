...
const DEFAULT_SECTION_ORDER = ['serviceTypes', 'equipment', 'commonIssues', 'brands', 'faqs'];
...
export default function ServicePageClient({ serviceData }: ServicePageClientProps) {
  ...
  const sectionOrder =
    svc.sectionOrder && svc.sectionOrder.length > 0
      ? svc.sectionOrder
  ...
  function renderSection(key: string) {
    ...
    if (key.startsWith('miniCta:')) {
      const ctaId = key.slice(8);
      const cta = svc.miniCtas.find((c: any) => c.id === ctaId);
      return cta ? <MiniCtaSection cta={cta} /> : null;
    }

    switch (key) {
      case 'serviceTypes':
        if (svc.serviceTypes.length === 0 || svc.sections?.serviceTypes === false) return null;
        return (
          <section className="py-16">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
              <h2 className="text-3xl font-bold text-gray-900 mb-12 text-center">
                Our {svc.name} Services
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {svc.serviceTypes.map((serviceType: any, index: number) => (
                  <div
                    key={index}
                    className="bg-gray-50 rounded-xl p-6 border border-gray-200 hover:border-blue-300 transition-colors"
                  >
                    <h3 className="text-xl font-bold text-gray-900 mb-4">{serviceType.name}</h3>
                    <p className="text-gray-600 mb-6">{serviceType.description}</p>
                    <ul className="space-y-3">
                      {serviceType.features &&
                        serviceType.features.map((feature: string, idx: number) => (
      ...
      case 'equipment':
        if (svc.equipment.length === 0 || svc.sections?.equipment === false) return null;
        return (
          <section className="py-16 bg-gray-50">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {svc.equipment.map((item: any, index: number) => (
                  <div
                    key={index}
      ...
      case 'commonIssues':
        if (svc.commonIssues.length === 0 || svc.sections?.commonIssues === false) return null;
        return (
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
              <div className="faq-container max-w-4xl mx-auto">
                ...
                <div className="faq-list space-y-4">
                  {svc.commonIssues.map((issue: any, index: number) => (
                ...
                <div className="faq-cta mt-12 text-center">
                  ...
                  <p className="faq-cta-text text-lg text-gray-600 mb-6">
                    Our certified technicians can diagnose and fix any {svc.name.toLowerCase()} issue quickly and efficiently.
                  </p>
                  <a
                    href="tel:405-242-6028"
                    className="faq-cta-button inline-flex items-center justify-center rounded-lg bg-blue-600 px-8 py-4 text-white font-semibold hover:bg-blue-700 transition-colors"
                  >
      ...
      case 'brands':
        if (svc.brands.length === 0 || svc.sections?.brands === false) return null;
        return (
          <section className="py-16 bg-white">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
              <div className="text-center mb-12">
                <h2 className="text-3xl font-bold text-gray-900 mb-4">Trusted Brands We Service</h2>
              ...
              <div className="hidden md:block overflow-hidden mb-8">
                <div className="flex animate-scroll">
                  {svc.brands.map((brand: any, index: number) => (
                  ...
                  {svc.brands.map((brand: any, index: number) => (
                    <div key={`dup-${index}`} className="flex-shrink-0 w-48 h-32 flex items-center justify-center px-4">
                      {brand.logo ? (
                        <div className="relative w-32 h-16">
                          <Image src={brand.logo} alt={`${brand.name} logo`} fill className="object-contain" sizes="128px" />
                        </div>
                  ...
                  <div key={index} className="h-24 flex items-center justify-center p-2">
                    {brand.logo ? (
                      <div className="relative w-full h-12">
                        <Image src={brand.logo} alt={`${brand.name} logo`} fill className="object-contain" sizes="(max-width: 640px) 100px, 128px" />
                      </div>
                    ) : (
      ...
      case 'faqs':
        if (svc.faqs.length === 0 || svc.sections?.faqs === false) return null;
        return (
          <section className="py-16">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
              <h2 className="text-3xl font-bold text-gray-900 mb-12 text-center">
              ...
              <div className="max-w-3xl mx-auto space-y-6">
                {svc.faqs.map((faq: any, index: number) => (
          ...
          <div className="flex flex-col lg:flex-row gap-12 items-center">
            <div className="max-w-3xl lg:flex-1">
              <div className="flex items-center space-x-3 mb-6">
                ...
                <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold">{svc.name}</h1>
              </div>
              <p className="text-xl text-gray-300 mb-8">{svc.description}</p>
                ...
                <div className="relative rounded-2xl overflow-hidden shadow-2xl">
                  ...
                  <div className="absolute bottom-6 left-6 right-6">
                    <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/20">
                      <p className="text-white font-semibold text-lg">Professional {svc.name} Service</p>
      ...
      <section className="py-16 bg-gradient-to-r from-blue-600 to-blue-800">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl mx-auto text-center text-white">
            <h2 className="text-3xl md:text-4xl font-bold mb-6">{svc.cta.title}</h2>
            <p className="text-xl mb-8">{svc.cta.description}</p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              ...
              <div className="bg-white/10 rounded-lg p-6">
                <div className="h-8 w-8 text-white mx-auto mb-4 flex items-center justify-center text-xl">⚡</div>
